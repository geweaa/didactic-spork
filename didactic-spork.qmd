...
## NMDS cultures: both initial and follow-up
...
p2 + p3 + p4 + plot_layout(nrow = 1, guides = "collect") + plot_annotation(tag_levels = "a") & theme(
...
```{r combine diversity plots amplicon data, fig.width=18/2.53, fig.height=10/2.53}
#| out-width: 100%
...
```{r}
p1 + px + plot_annotation(tag_levels = "a") & theme(
  plot.tag.location = "plot",
...
```{r export area plot}
ggsave("figures/fig-sx.pdf", height = 11, width = 18, units = "cm")
```
...
```{r}
ggsave(filename = "figures/fig-s2.pdf", width = 180, height = 180, units = "mm")
```
...
mw <- mw %>% 
  ...
  pathway == "Organic carbon oxidation - methanol oxidation" ~ "04: Organic C: methanol oxidation",
  pathway == "Organic carbon oxidation - fatty acid degradation" ~ "05: Organic C: fatty acid degradation",
...
ggsave("figures/fig-x.pdf", width = 140, height = 148, units = "mm")
```
