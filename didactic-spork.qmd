...
ggsave("figures/fig-s5.pdf", width = 160, height = 160, units = "mm")
```
...
ggsave("figures/fig-s6.pdf", width = 140, height = 160, units = "mm")
```
...
t <- data.frame(x = 0, y = 6, type = "Groundwaters",
                label = "Patescibacteria: 127 nodes\nDesulfobacterota: 38\nNanobdellota: 38\nOmnitrophota: 17\nChloroflexota: 11\nAtribacterota: 6\nBacteroidota: 4\nCampylobacterota: 4")
...
g %>%
  ...
  geom_text(data = l, aes(x = x, y = y, label = label), size = 7/.pt, hjust = -0.1, fontface = "bold", vjust = 1.2) +
  facet_nodes(~type, scales = "free") +
  scale_fill_manual(values = cols.phylum) +
  theme_graph(
    base_family = "Helvetica", 
    base_size = 5,
    plot_margin = margin(l = 1, r = 1)
    ) +
  theme(
...
```{r, eval=FALSE}
ggsave("figures/network.pdf", width = 180, height = 120, units = "mm")
```
...
```{r, eval=FALSE}
ggsave("figures/fig-s3.pdf", height = 90, width = 180, units = "mm")
```
...
```{r, eval=FALSE}
ggsave(filename = "figures/fig-s4.pdf", width = 180, height = 180, units = "mm")
```
...
# Supplementary table metagenomic assemblies
