...
knitr::opts_chunk$set(echo = TRUE, cache = TRUE, dpi = 40)
```
...
seqtab <- read_tsv("data/ampliseq/ASV_table.tsv.gz", col_types = cols(.default = col_character(), 
                                    count = col_integer()
                                    )) %>%
  group_by(sample) %>% mutate(relab = count / sum(count)) %>% ungroup()

tax <- read_tsv("data/ampliseq/ASV_tax.tsv.gz", col_types = cols(.default = col_character())) %>%
  mutate(phylum = if_else(phylum == "Nanoarchaeota", "Nanobdellota", phylum))
               
smd <- read_tsv("data/ampliseq/metadata.tsv", col_types = cols(.default = col_character())) %>%
...
# P21015_1032 was the extraction control for the t0 sampling
# P21015_1033 was the extraction control for the incubations
...
c <- seqtab %>% filter(sample == 'P21015_1032') %>% pull(seqid) %>% unique()
# P21015_1032 contains 376 ASVs that will be removed from the t0 samples

i <- seqtab %>% 
  filter(sample %in% t0 & !seqid %in% c)

seqtab <- seqtab %>%
  filter(!sample %in% t0) %>%
  rbind(i)

# A similar operation for the incubations
c <- seqtab %>% filter(sample == 'P21015_1033') %>% pull(seqid) %>% unique()

x <- smd %>% filter(fraction == 'fractionated' | fraction == 'non-fractionated') %>% pull(sample)

i <- seqtab %>% 
  filter(sample %in% x & !seqid %in% c)
...
# P21015_1085 contained 141 ASVs, P21015_1086 183 ASVs. Both are acetate media controls
c <- seqtab %>% filter(sample %in% c('P21015_1085', 'P21015_1086')) %>% pull(seqid) %>% unique()

x <- smd %>% filter(medium == 'ace' & fraction != 'control') %>% pull(sample)
...
# Similar operation for the lysate media controls
# P21015_1090 contained 233 ASVs and P21015_1091 contained 220 ASVs
c <- seqtab %>% filter(sample %in% c('P21015_1090', 'P21015_1091')) %>% pull(seqid) %>% unique()
x <- smd %>% filter(medium == 'lys' & fraction != 'control') %>% pull(sample)
...
This project includes 15 metagenomes from 15 unique cultures. The metagenomes contain 35 de-replicated metagenome-assembled genomes (MAG) that are affiliated with the Bacillota (7), Desulfobacterota (7), Pseudomonadota (6), Bacteroidota (4), Patescibacteria (3), Nanobdellota (2), Acidobacteriota (1), Campylobacterota (1), Chloroflexota (1), Iainarchaeota (1), Spirochaetota (1), and Verrucomicrobiota (1).


```{r files metag}
coverm <- read_tsv("data/mag/coverm-mapping.tsv", col_types = 'ccd')

emapper <- read_tsv("data/mag/emapper.tsv.gz", col_types = 'ccdcccc') 

# Sample 35 (l7f) was removed from the binning process as this sample did not produce any bins
bintax <- read_tsv("data/mag/gtdbtk.summary.tsv", col_types = cols(.default = col_character())) %>%
  mutate(phylum = if_else(phylum == "Nanoarchaeota", "Nanobdellota", phylum))

quality <- read_tsv("data/mag/quality_report.tsv", show_col_types = F) %>%
  rename_with(tolower) %>%
  rename(genome = name) %>%
  filter(genome %in% coverm$mag)

# Read data from KEGG mapper with 'c' as complete, 'i' as incomplete, and 'm' as module missing
km <- read_tsv('data/mag/kegg-mapper.tsv.gz', col_types = 'ccc')
```
...
chem <- read_tsv("data/sicada.tsv", show_col_types = FALSE) %>% 
  rename_with(tolower) %>% 
  filter(sampling) %>%
...
p2 <- rda$CCA$wa %>% data.frame() %>% tibble::rownames_to_column('sample') %>%
  ...
  scale_size(range = c(1,8), name = "No. of ASVs", breaks = c(1000, 2000)) +
  annotate("text", x = -0.3, y =-0.16, label = "Meteoric", colour = cols.gw[1], size = 7/.pt) +
  annotate("text", x = 0.04, y = 0.22, label = "Marine", colour = cols.gw[2], size = 7/.pt) +
...
p3 <- seqtab %>%
  ...
  # Summarize in order to have the sum for each category and phylum
...
p4 <- adiv %>%
  ...
  geom_point(position = position_jitterdodge(
    ...
    size = 1.5, shape = 21, stroke = 0.5, key_glyph = "boxplot") +
...
ggsave("figures/fig-1.pdf", width = 180, height = 160, units = "mm")
```
...
```{r}
#| label: lm-groundwater

t <- c(
  "Spirochaetota", "Acidobacteriota", "Atribacterota", "Chloroflexota",
  "Omnitrophota", "Nanobdellota", "Campylobacterota", "Pseudomonadota"
)

# This object is the amplicon data grouped by phylum from https://doi.org/10.3389/fmicb.2018.02880
network.survey <- read_tsv("data/ampliseq/network-survey.tsv", show_col_types = F)
...
p1 <- network.survey %>%
  ...
  geom_label(data = fit %>% mutate(phylum = factor(phylum, levels = phylum)), 
            ...
            size = 5/.pt, hjust = 1, vjust = 0, parse = TRUE, label.size = 0, fill = "white") +
...
p2 <- network %>%
  ...
  geom_label(data = n, 
            ...
            size = 5/.pt, hjust = 1.0, vjust = -0.0, parse = TRUE, label.size = 0, fill = "white") +
  geom_smooth(method = "lm", formula = y ~ x, colour = "black", fill = "#D9D9D9", linewidth = 0.5) + 
  labs(x = "Sqrt relative abundance Patescibacteria (%)", y = "Sqrt relative abundance (%)") +
  theme_tidy(fontsize = 5) +
  scale_y_continuous(labels = function(x) sprintf("%.1f", x)) +
...
ggsave("figures/fig-s6.pdf", width = 160, height = 160, units = "mm")
```
...
survey.amplicons <- read_tsv("data/ampliseq/survey-amplicons.tsv.gz", show_col_types = FALSE) %>%
  group_by(sample) %>% mutate(relab = count / sum(count)) %>% ungroup()
...
label <- c(
  ...
  "Limnocylindrales UBA1558" = "Limnocylindrales\n-\nUBA1558"
  )
...
label = c(
  "Gygaellales UBA10092" = "Gygaellales\n-\nMagasanikbacterales",
  "Dehalococcoidales Magasanikbacterales" = "Dehalococcoidales\n-\nMagasanikbacterales",
  "Gygaellales Paceibacterales" = "Gygaellales\n-\nPaceibacterales",
  "Gygaellales UBA9973" = "Gygaellales\n-\nUBA9973",
  "Gygaellales Magasanikbacterales" = "Gygaellales\n-\nMagasanikbacterales",
  "Dehalococcoidales Paceibacterales" = "Dehalococcoidales\n-\nPaceibacterales",
  "Dehalococcoidales Portnoybacterales" = "Dehalococcoidales\n-\nPortnoybacterales",
  "Treponematales UBA10092" = "Treponematales\n-\nUBA10092"
)
...
```{r export network analysis, eval=FALSE}
ggsave("figures/fig-s7.pdf", width = 140, height = 160, units = "mm")
```

# Negative controls

## Negative controls: phylum

```{r}
#| label: identify top phyla

t <- seqtab %>%
  filter(sample %in% c("P21015_1085", "P21015_1086", "P21015_1090", "P21015_1091", "P21015_1032", "P21015_1033")) %>%
  inner_join(tax, by = "seqid") %>%
  group_by(phylum, sample) %>%
  # Sum the abundance of each phylum within a sample
  summarise(relab = sum(relab), .groups = 'drop_last') %>%
...
p.phylum <- seqtab %>%
  ...
  labs(x = '', y = 'Relative abundance (%)', fill = "") +
  geom_col() + scale_y_continuous(expand = c(0.01,0), limits = c(-6,101)) +
  annotate(geom = "segment", x =  0.55, xend =  2.45, y = -1, yend = -1, linewidth = 0.5) +
  annotate(geom = "segment", x =  2.55, xend =  4.45, y = -1, yend = -1, linewidth = 0.5) +
  annotate(geom = "segment", x =  4.55, xend =  6.45, y = -1, yend = -1, linewidth = 0.5) +
  # Label controls
  annotate(geom = "text", x =  1.5, y = -3.5, label = "Extraction controls", size = 5/.pt) +
  annotate(geom = "text", x =  3.5, y = -3.5, label = "Acetate controls", size = 5/.pt) +
  annotate(geom = "text", x =  5.5, y = -3.5, label = "Lysate controls", size = 5/.pt) +
  annotate(geom = "text", x =  1, y = 3.5, label = "376 ASVs", size = 5/.pt) +
  annotate(geom = "text", x =  2, y = 3.5, label = "358", size = 5/.pt) +
  annotate(geom = "text", x =  3, y = 3.5, label = "141", size = 5/.pt) +
  annotate(geom = "text", x =  4, y = 3.5, label = "183", size = 5/.pt) +
  annotate(geom = "text", x =  5, y = 3.5, label = "233", size = 5/.pt) +
  annotate(geom = "text", x =  6, y = 3.5, label = "220", size = 5/.pt) +  
  scale_fill_manual(values = cols.phylum) + 
  theme_tidy(legend = "bottom", fontsize = 5) +
...
## Negative controls: order

```{r}
#| label: identify top order 

c <- c("P21015_1085", "P21015_1086", "P21015_1090", "P21015_1091", "P21015_1032", "P21015_1033")
...
cols.order <- c(
  "Burkholderiales" = "#972D15",
  "Holophagales" = "#FAEFD1",
  "Mycobacteriales" = "#C6CDF7",
  "Treponematales" = "#D8B70A",
  "PCC-6307" = "#046C9A",
  "Pseudomonadales" = "#A2A475",
  "Actinomycetales" = "#1B5331",
  "Caulobacterales" = "#C7B19C",
  "Other" = "#899DA4"
)
```
...
p.order <- seqtab %>%
  ...
  ggplot(aes(x = sample, 
             y = relab * 100, 
             fill = fct_relevel(order, names(cols.order) %>% rev())
             )
         ) +
  labs(x = '', y = 'Relative abundance (%)', fill = "") +
  geom_col() + scale_y_continuous(expand = c(0.01,0), limits = c(-6,101)) +
...
```{r}
p.phylum + p.order + plot_annotation(tag_levels = "a") &
  theme(
    plot.tag.location = "panel",
    plot.tag.position = c(-0.1, 0.99)
  )

remove(p.phylum, p.order)
```

```{r}
ggsave(filename = "figures/controls.pdf", width = 180, height = 80, units = "mm")
```


# Network analysis ASV


```{r}
#| label: prepare data for correlation
...
s <- c(t0, unique(survey.amplicons$sample))

d <- seqtab %>%
  inner_join(tax, by = "seqid") %>%
  # Add the data from groundwaters at Äspö HRL
  rbind(survey.amplicons) %>% 
  filter(phylum %in% names(cols.phylum)) %>%
  select(seqid.y = seqid, sample, relab.y = relab) 

d <- seqtab %>%
  inner_join(tax, by = "seqid") %>%
  rbind(survey.amplicons) %>%
  filter(phylum %in% names(cols.phylum)) %>%
  select(seqid, sample, relab) %>%
  inner_join(d, by = "sample") %>%
  # Minimum read count of 0.05 % 
  filter(sample %in% s & relab > 0.005) 
```


```{r}
#| label: correlation-groundwater

cor <- tibble()

for (x in unique(d$seqid)) {
     n <- d %>% 
       filter(seqid == x) %>%
       group_by(seqid.y) %>% 
       summarise(spearman = cor(relab, relab.y, method = "spearman"), n = n()) %>%
       mutate(seqid = x) %>%
       mutate(type = "Groundwaters") %>%
       filter(!is.na(spearman) & n > 10 & spearman > 0)
     cor <- rbind(cor, n)
      }
```
...
```{r}
#| label: data-correlation-cultures
...
d <- seqtab %>%
  inner_join(tax, by = "seqid") %>%
  # Only use samples from non-fractionated cultures
  filter(phylum %in% names(cols.phylum) & sample %in% s) %>%
  select(seqid.y = seqid, sample, relab.y = relab) 

# By running inner_join() on the samples, all combinations between seqid's are obtained
d <- seqtab %>%
  inner_join(tax, by = "seqid") %>%
  filter(phylum %in% names(cols.phylum) & sample %in% s) %>%
  select(seqid, sample, relab) %>%
  inner_join(d, by = "sample") %>%
  # Minimum read count of 0.1 % 
  filter(sample %in% s & relab > 0.005) 
```

```{r}
#| label: correlation-cultures
...
for (x in unique(d$seqid)) {
     n <- d %>% 
       ...
       summarise(spearman = cor(relab, relab.y, method = "spearman"), n = n()) %>%
       mutate(seqid = x) %>%
       mutate(type = "Cultures") %>%
       # n > 10 implies at least ten co-occurrences
       filter(!is.na(spearman) & n > 3 & spearman > 0)
     cor <- rbind(cor, n)
      }
```
...
```{r}
g <- cor %>% filter(spearman > 0.75) %>%
  filter(seqid != seqid.y) %>%
  rename(from = seqid, to = seqid.y) %>%
  select(from, to, n, spearman, type)

taxref <- survey.amplicons %>%
  filter(!seqid %in% tax$seqid) %>%
  select(-sample, -count, -relab) %>%
  rbind(tax) %>%
  distinct()
...
a <- g %>%
  select(type, from, to) %>%
  pivot_longer(-1, names_to = "relation", values_to = "seqid") %>%
  select(-relation) %>%
  left_join(taxref, by = "seqid") %>%
  select(seqid, phylum, type) %>%
  distinct(seqid, .keep_all = TRUE)

g <- igraph::graph_from_data_frame(g, directed = FALSE, vertices = a)
...
t <- data.frame(x = 0, y = 6, type = "Groundwaters",
                label = "Patescibacteria: 142 nodes\nDesulfobacterota: 48\nBacteroidota: 25\nOmnitrophota: 17\nChloroflexota: 10\nBacillota: 6\nCampylobacterota: 5\nAtribacterota: 2\nNitrospirota: 2")

l <- data.frame(x = rep(-Inf, 2), y = rep(Inf, 2), type = c("Groundwaters", "Cultures"),
                label = c("a: groundwaters", "b: incubations"))

g %>%
  tidygraph::activate(nodes) %>%
  mutate(group = as.factor(tidygraph::group_infomap())) %>%
  mutate(type = factor(type, levels = c("Groundwaters", "Cultures"))) %>%
  mutate(phylum = factor(phylum, levels = names(cols.phylum))) %>%
  ggraph(layout = 'stress') +
  geom_edge_fan(width = 0.2) + 
  geom_node_point(aes(fill = phylum), shape = 21, stroke = 0.3, size = 2.4) +
  geom_text(data = t, aes(x = x, y = y, label = label), size = 6/.pt, hjust = 0) +
  geom_text(data = l, aes(x = x, y = y, label = label), size = 7/.pt, hjust = -0.1, fontface = "bold", vjust = 1.3) +
  facet_nodes(~type, scales = "free") +
  scale_fill_manual(values = cols.phylum) +
  theme_graph(
    base_family = "Helvetica", 
    base_size = 5,
    plot_margin = margin(l = 1, r = 1, t = 1)
    ) +
  theme(
...
```{r, eval=FALSE}
ggsave("figures/fig-6.pdf", width = 180, height = 100, units = "mm")
```
...
nmds <- seqtab %>% # Full size fraction
  inner_join(smd, by = "sample") %>%
  filter(!is.na(age) | sample %in% t0 & groundwater == "KR0015" | fraction == "control") %>%
...
p1 <- nmds.scores %>%
  ...
  mutate(fraction = factor(fraction, levels = 
                             c("environmental","fractionated","non-fractionated","control"))) %>%
  ...
  geom_point(size = 4.0, stroke = 0.7) + 
  geom_text(data = nmds.scores, 
            aes(label = age), 
            size = 5/.pt, color = "black", na.rm = TRUE) + 
  scale_shape_manual(values = c("ace" = 23, "lys" = 24, "env" = 21), 
                     labels = c("Acetate", "Lysate", "Environmental")) +
  scale_color_manual(values = cols.fraction, labels = c("Environmental", "< 0.45 µm", "Non-fractionated", "Media control")) +
  scale_fill_manual(values = alpha(cols.fraction, alpha = 0.2), guide = "none") +
  annotate('text', x = -Inf, y = -Inf, size = 5/.pt, hjust = -0.1, vjust = -0.5, 
           label = paste('Stress = ', round(nmds$stress, digits = 2))) +
  theme_tidy(legend = "bottom", fontsize = 5) +
  ggforce::geom_circle(aes(x0 = -1.6, y0 = 0.15, r = 0.45),
              data = i,
              inherit.aes = FALSE,
...
p2 <- seqtab %>%
  ...
  mutate(phylum = factor(phylum, levels = rev(names(cols.phylum)))) %>%
  # Call the plot
  ggplot(aes(
    age, 
    relab * 100, 
...
```{r, eval=FALSE}
ggsave("figures/fig-s4.pdf", height = 90, width = 180, units = "mm")
```
...
p1 <- qpcr %>%
  ...
  summarise(mean = mean(quantity), sd = 2 * sd(quantity), .groups = 'drop') %>%
  inner_join(smd, by = "sample") %>%
  ...
  geom_pointrange(aes(ymin = mean - sd, 
                      ymax = mean + sd), 
                  size = 2, shape = 21, stroke = 0.5, fatten = 1) +
  ...
  scale_linetype(name = "Medium:", labels = c("Acetate", "Cell lysate"), guide = "none") +
...
```{r, eval=FALSE}
ggsave(filename = "figures/fig-s5.pdf", width = 180, height = 180, units = "mm")
```
...
cols.phylum <- c(
  ...
  "Iainarchaeota" = "#00A08A",
  "Other" = "#899DA4"
)

t <- c("Bacillota","Desulfobacterota","Pseudomonadota","Patescibacteria","Nanobdellota","Other")
t <- names(cols.phylum)
...
p1 <- i %>%
  ...
  geom_point(size = 3, shape = 21, stroke = 0.5, key_glyph = "rect") + 
...
p2 <- i %>%
  mutate(phylum = case_when(
    phylum %in% c("Nanobdellota", "Patescibacteria", "Bacillota", "Desulfobacterota", "Pseudomonadota") ~ phylum,
    .default = "Other"
  )) %>%
  mutate(phylum = factor(phylum, levels = c(
    "Nanobdellota", "Patescibacteria", "Other", "Bacillota", "Desulfobacterota", "Pseudomonadota"
  ))) %>%
  ggplot(aes(
    phylum, 
    completeness, 
  ...
  scale_fill_manual(values = alpha(cols.phylum, alpha = 0.5), guide = "none") + 
...
p4 <- nmds.scores %>%
  ...
  scale_color_manual(values = cols.phylum, name = "Phylum", guide = "none") +
  scale_fill_manual(values = alpha(cols.phylum, alpha = 0.5), name = "", guide = "none") +
...
```{r export fig on binstats, eval=FALSE}
...
km <- km %>% mutate(label = case_when(
    ...
    module == "M00052" ~ "M00052: Ribonucleotide biosynthesis",
    .default = NA
  ))

t <- tibble(
  x = c(0.5, 7.55, 13.55, 20.55, 23.55, 25.55, 26.55, 27.55, 28.55),
  xend = c(7.45, 13.45, 20.45, 23.45, 25.45, 26.45, 27.45, 28.45, 29.45),
  y = rep(14.3, 9), 
  yend = rep(14.3, 9),
  phylum = c("Desulfobacterota","Pseudomonadota","Bacillota","Patescibacteria","Nanobdellota",
             "Chloroflexota","Spirochaetota","Campylobacterota","Iainarchaeota")
  ) %>%
  mutate(phylum = factor(phylum, names(cols.phylum)))
...
d <- quality %>% inner_join(bintax, by = "genome") %>%
  mutate(genus = case_match(genus, NA ~ family, .default = genus)) %>%
  mutate(taxon = if_else(is.na(species), paste(genus, "sp."), species)) %>%
  mutate(taxon = factor(taxon, levels = name)) %>% filter(!is.na(taxon)) %>%
  select(taxon, completeness) %>% arrange(taxon) %>%
  mutate(x = seq(1,29, by = 1)) %>%
  mutate(y = rep(13.8, 29))
...
p1 <- km %>%
  ...
  filter(!is.na(label)) %>%
  # Add genome completeness
  inner_join(quality, by = "genome") %>%
  ggplot(aes(
    x = taxon,
    y = fct_rev(label),
    fill = score
  ...
  theme_tidy(fontsize = 5) + coord_fixed() +
  geom_text(data = d, aes(x = x, y = y, label = round(completeness, digits = 0)), inherit.aes = FALSE, size = 5/.pt) +
  geom_vline(xintercept =  7.5, linewidth = 0.6, color = "white") +
  geom_vline(xintercept = 13.5, linewidth = 0.6, color = "white") +
  geom_vline(xintercept = 20.5, linewidth = 0.6, color = "white") +
  geom_vline(xintercept = 23.5, linewidth = 0.6, color = "white") +
  geom_vline(xintercept = 25.5, linewidth = 0.6, color = "white") +
  geom_segment(data = t, aes(x = x, xend = xend, y = y, yend = yend, color = phylum), linewidth = 2, inherit.aes = FALSE) +
  scale_color_manual(values = cols.phylum, guide = "none") +
  theme(
    legend.position = "bottom",
    ...
    legend.text = element_text(margin = margin(l = 2)),
    legend.margin = margin(t = -10),
    axis.text.x.top = element_text(angle = 90, hjust = 0, vjust = 0.6, size = 5),
...
p2 <- mw %>%
  ...
  geom_tile(color = "white") +
  scale_fill_manual(
    values = c("c" = "#707151", "m" = "#F0F1EB"),
    label = c("c" = "Present", "m" = "Absent"), 
    guide = "none") +
  theme_tidy(fontsize = 5) + coord_fixed() +
  geom_vline(xintercept =  7.5, linewidth = 0.6, color = "white") +
  geom_vline(xintercept = 13.5, linewidth = 0.6, color = "white") +
  ...
  geom_vline(xintercept = 25.5, linewidth = 0.6, color = "white") +
  geom_segment(data = t, aes(x = x, xend = xend, y = y, yend = yend, color = phylum), linewidth = 2, inherit.aes = FALSE) +
  scale_color_manual(values = cols.phylum) +
  theme(
    axis.title.x = element_blank(),
    axis.title.y = element_blank(),
...
```{r}
i <- emapper %>% mutate(gene = case_when(
  grepl("1.6.5.3", ec) ~ "NADH:ubiquinone oxidoreductase",
  grepl("1.2.5.3", ec) ~ "Aerobic CO dehydrogenase",
  grepl("1.9.3.1", ec) ~ "Cytochrome c oxidase",
  grepl("3.6.3.14", ec) ~ "F-type H+/Na+-transporting ATPase",
  .default = "Other"
  )) %>%
  group_by(genome, gene) %>% summarise(n = n(), .groups = "drop") %>%  
  pivot_wider(names_from = "gene", values_from = "n", values_fill = 0) %>%
  pivot_longer(-1, names_to = "gene", values_to = "n")
```
...
p3 <- i %>%
  ...
  geom_tile(color = "white") +
  scale_fill_manual(values = c("1" = "#707151",
                               "0" = "#F0F1EB"),
                    label = c("1" = "Present",
                              "0" = "Absent")) +  
  theme_tidy(fontsize = 5) + coord_fixed() + guides(fill = guide_legend(reverse = TRUE)) +
  geom_vline(xintercept =  7.5, linewidth = 0.6, color = "white") +
  geom_vline(xintercept = 13.5, linewidth = 0.6, color = "white") +
  geom_vline(xintercept = 20.5, linewidth = 0.6, color = "white") +
  geom_vline(xintercept = 23.5, linewidth = 0.6, color = "white") +
...
p1 / p2 / p3 + plot_annotation(tag_levels = 'a') & theme(
...
```{r export plot metabolic weight, eval=FALSE}
