...
```{r files metag}
coverm <- data.frame() # Soon will contain 33 de-replicated genomes
...
# MEGAHIT-MaxBin2Refined-VK-3516-a1u_S32_L002.006 in taxonomy and checkm but not in coverm output
...
```{r plot environmental communities (t0), fig.width = 12/2.54}
seqtab %>%
  filter(sample %in% t0) %>%
  inner_join(taxref, by = "seqid") %>%
  # Summarize in order to have the sum for each category and topphylum
  ...
  geom_col() + scale_y_continuous(expand = c(0.01,0), limits = c(-8,101)) +
  ...
  # Labels dates
  annotate(geom = "text", x =  2, y = -3, label = "Oct 2019", size = 7/.pt) +
  annotate(geom = "text", x =  5, y = -3, label = "April 2020", size = 7/.pt) +
  annotate(geom = "text", x =  8, y = -3, label = "Oct 2019", size = 7/.pt) +
  annotate(geom = "text", x = 11, y = -3, label = "April 2020", size = 7/.pt) +
  annotate(geom = "text", x = 14, y = -3, label = "Oct 2019", size = 7/.pt) +
  annotate(geom = "text", x = 17, y = -3, label = "April 2020", size = 7/.pt) +
  # Labels groundwater
  annotate(geom = "text", x = 3.5, y = -7, label = "KR0015", size = 7/.pt) +
  annotate(geom = "text", x = 9.5, y = -7, label = "SA1420", size = 7/.pt) +
  annotate(geom = "text", x =15.5, y = -7, label = "SA2600", size = 7/.pt) +
...
ggplot() +
  ...
  lims(x = c(11,21), y = c(55,61)) + labs(x = "", y = "") + theme_tidy(ratio = NULL, fontsize = 6) +
...
```{r export se-sweden}
ggsave("figures/se-sweden-void.png", height = 6, width = 6, units = "cm")
```
...
p1<- i %>%
  ...
  scale_colour_manual(values = cols.phylum.mag) +
...
p2 <- i %>%
  ...
  scale_fill_manual(values = cols.phylum.mag, guide = "none") + 
  scale_color_manual(values = cols.phylum.mag, guide = "none") +
...
p3 <- i %>%
  ...
  scale_colour_manual(values = cols.phylum.mag, guide = "none") +
  theme_tidy() + scale_x_continuous(breaks = c(1,2,3,4,5,6)) +
  annotate("text", x = Inf, y = Inf, 
