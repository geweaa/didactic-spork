...
smd <- read_tsv("smd.P21015.P26201.tsv", col_types = cols(.default = col_character())) %>%
  mutate(label = paste(medium," (",substr(fraction,1,1),")", sep = ""))

seqtab %>% inner_join(smd, by = "sample") %>% filter(age == "ctrl" & medium == "ace") %>% pull(seqid) %>% unique() -> ace
seqtab %>% inner_join(smd, by = "sample") %>% filter(age == "ctrl" & medium == "lys") %>% pull(seqid) %>% unique() -> lys
...
```{r lollipop interest}
seqtab %>% # Select the cultures of interest
  inner_join(tax, by = "seqid") %>%
  filter(phylum %in% c("Patescibacteria","Nanoarchaeota")) %>%
  ...
  inner_join(smd, by = "sample") %>%
  top_n(30, mean_relab) %>%
  arrange(mean_relab) -> hot

seqtab %>% # Prepare the labels
  inner_join(tax, by = "seqid") %>%
  group_by(sample, taxon) %>% summarise(relab = sum(relab) * 100, .groups = "drop_last") %>%
  slice_max(order_by = relab, n = 1) %>% ungroup() %>%
  mutate(i = paste(taxon, " (", round(relab, digits = 1), "%)", sep = "")) %>%
  mutate(hjust = if_else(relab >= 50, 1.05, -0.07)) %>%
  inner_join(smd, by = "sample") -> plotlab
  
plotlab %>%
  filter(sample %in% hot$sample) %>%
  ggplot(aes(x = fct_relevel(sample, hot$sample), 
             y = relab)) +
  ...
  coord_flip() +
  scale_x_discrete(labels = hot$label) +
  labs(x = "", y = "") +
  theme_tidy() +
...
seqtab %>%
  ...
  theme(aspect.ratio = 2, 
        ...
        legend.key.size = unit(3.2, "mm")) -> p2
```
...
ggsave(filename = "figures/patesci.png", width = 20, height = 14, units = c("cm"))
```
